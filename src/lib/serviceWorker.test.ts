...
function worker(clients: object[] = []) {
  ...
  const caches={keys:vi.fn().mockResolvedValue(["unrelated-cache","garma-shell-v6","garma-shell-v7"]),delete:vi.fn(),match:vi.fn(),open:vi.fn()};
  const fetch=vi.fn().mockRejectedValue(new Error("offline"));
  vm.runInNewContext(readFileSync(resolve("public/sw.js"),"utf8"),{self,caches,fetch,URL,Response,Date});
...
it("activation does not delete other apps' caches",async()=> {
  const {fire,caches}=worker();await fire("activate");expect(caches.delete).toHaveBeenCalledExactlyOnceWith("garma-shell-v6");
});
