...
async function setup() {
  ...
  const a=(await t.mutation(api.users.register,{token:"status-a",displayName:"A"})).user;
  const b=(await t.mutation(api.users.register,{token:"status-b",displayName:"B"})).user;
  const storageId=await t.run(ctx=>ctx.storage.store(new Blob(["image bytes"],{type:"image/jpeg"})));
  const first=await t.mutation(api.statuses.post,{token:"status-a",kind:"image",storageId});
...
it("deleting a status does not destroy a photograph used in another person's status",async()=>{
  ...
  await t.mutation(api.statuses.post,{token:"status-b",kind:"image",storageId});
  await t.mutation(api.statuses.remove,{token:"status-a",statusId:first});
...
it("deleting a status does not destroy the same photograph in a conversation",async()=>{
  ...
  const conversationId=await t.mutation(api.conversations.startDM,{token:"status-a",otherId:b._id});
  await t.mutation(api.messages.send,{token:"status-a",conversationId,kind:"image",storageId,body:"shared"});
  await t.mutation(api.statuses.remove,{token:"status-a",statusId:first});
  expect(await t.run(async ctx=>Boolean(await ctx.storage.get(storageId)))).toBe(true);
});
...
it("expiry housekeeping releases images after their final reference expires",async()=>{
  ...
  await t.mutation(api.statuses.cleanupExpired,{token:"status-a"});
  expect(await t.run(async ctx=>Boolean(await ctx.storage.get(storageId)))).toBe(false);
});
it("a busy global feed cannot hide a person's own status",async()=>{
  ...
  expect((await t.query(api.statuses.mine,{token:"status-a"})).map((r:any)=>r._id)).toContain(first);
});
it("view receipts are idempotent rather than changing on every reactive delivery",async()=>{
  const {t,first}=await setup();
  await t.mutation(api.statuses.view,{token:"status-b",statusId:first});
